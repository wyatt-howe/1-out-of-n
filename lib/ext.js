module.exports = function (ot, io, sodium, util, crypto) {
  ...
  ot.extension.seed_and_execute_sender = function (n, k, X, op_id, session_id) {
    ...
    const Q_promises = (function seed_stage_sender() {
      // Seed OT
      const Q_promises = s.map((s_j, j) =>
    ...
    Promise.all(Q_promises).then(function (Q) {
      (function execute_stage_sender() {
        ...
        var i;
        for (i = 0; i < n; i++) {
          const row_i = Q.map(col => col[i]);
          const y_i_0 = xor_fit(X[i][0], sodium.crypto_generichash(k_BYTES, new Uint8Array([i].concat(row_i))));
          const y_i_1 = xor_fit(X[i][1], sodium.crypto_generichash(k_BYTES, new Uint8Array([i].concat(row_i.map((Q_i_j, j) => Q_i_j ^ s[j])))));
  ...
  ot.extension.seed_and_execute_receiver = function (n, k, r, __listeners, op_id, session_id) {
    ...
    (function seed_stage_receiver() {
      ...
      var j;
      for (j = 0; j < k; j++) {
        const col_j = T.map(row => row[j]);
        const m0 = col_j;
        const m1 = col_j.map((col_j_i, i) => col_j_i ^ r[i]);
