module.exports = function (sodium) {
  ...
  const del = ', ';
  const esc = '\\';
  const delimit = s => s.join(' ' + del);
  const limit = s => s.split(' ' + del);
  const escape = s => s.split(del).join(esc + del);
  const unescape = s => s.split(' ' + esc + del).join(' ' + del);
  ...
  const to_str = function (x) {
    if (Array.isArray(x)) {
      return delimit(x.map(to_str).map(escape));
    } else {
      return from_Uint8(x);
    }
  };
  const from_str = function (str) {
    if (str.indexOf(del) > 1) {
      var arr = limit(str).map(unescape);
      var json_array = '["' + arr.join('","') + '"]';
